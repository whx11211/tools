...
define('B_VALUE','//');     //值前分割符
define('A_VALUE', '');      //值后分割符
            ...
            <?php 
                foreach ($msg_arr as $v) {
                    if (($b_key && strpos($v, $b_key) !== false) || ($a_key && strpos($v, $a_key) !== false)) {//可以匹配到键值，开始处理
                        ...
                        //值处理
                        $s_sub = ($b_value && strpos($v, $b_value) !== false) ?  (strpos($v, $b_value) + strlen($b_value)) : 0;
                        $e_sub = ($a_value && strpos($v, $a_value) !== false) ? strpos($v, $a_value) : 0;
                        if ($e_sub && ($e_sub - $s_sub) > 0) {
                            $value = trim(substr($v, $s_sub, $e_sub - $s_sub));
                        }
                        else {
                            $value = trim(substr($v, $s_sub));
                        }
                        if (!is_numeric($id)) {
                            $id = "'$id'";
                        }
                        //键留空计算
                        $count = $key_len - strlen($id) >= 0 ? $key_len - strlen($id) : 0;
                        echo '&nbsp&nbsp&nbsp&nbsp'.$id.str_repeat('&nbsp', $count).'=>&nbsp&nbsp\''.$value.'\','.'<br/>';
                    }
                    else if($v) {
                        echo '//'.$v;
                        echo '<br/>';
